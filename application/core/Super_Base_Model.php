...
abstract class Super_Base_Model extends CI_Model
{
    ...
    public function insert($datas, $extra_param = array())
    {
        ...
        (isset($extra_param["is_version"])) ? $is_version = $extra_param["is_version"] : $is_version = false;
        (isset($extra_param["ordering"])) ? $ordering = $extra_param["ordering"] : $ordering = false;
        ...
        if (!$is_batch) {
            ...
            if ($is_version) {
                //get last version for this table
                $last_ver = get_last_ver($this->_table) + 1;
                $datas['version'] = $last_ver;
            }

            if ($ordering) {
                //get last version for this table
                $last_ordering = get_last_ordering($this->_table) + 1;
                $datas['ordering'] = $last_ordering;
            }
    ...
    /**
     * function replace.
     * it means, will replace the data if the primary key exists.
     * or will insert as new if not exists.
     */
    public function replace($datas, $extra_param = array())
    {
        (isset($extra_param["is_direct"])) ? $is_direct = $extra_param["is_direct"] : $is_direct = false;

        if (!$is_direct) {
            // $datas['last_updated_by'] = $this->session->sess_login_admin['user_id'];
            // $datas['updated_date'] = date("Y-m-d H:i:s");
        }

        return $this->db->replace($this->_table, $datas);
    }
