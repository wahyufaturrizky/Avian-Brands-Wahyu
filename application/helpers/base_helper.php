...
// get region information by ip using ipstack API.
function get_client_province_by_ip($ip) {
    $url = "http://api.ipstack.com/" . $ip . "?access_key=5e57247992e61e0633690b08ef40b19a";
...
function validate_date_input($date) {
    ...
    if ($date) {
    	...
    	return ($date != "" && $date != "1900-01-01") ? $date : null;
    } else {
        return null;
    }
}
 ...
 * FUNCTION TO VALIDATE DATE FORMAT TO INSERT or UPDATE to database.
 */
function validate_date_time_input($date) {
    //do some conversion from d/m/Y format to strtotime compatible.
    $date = DateTime::createFromFormat('d/m/Y H:i:s', $date);

    if ($date) {
        $date = $date->format("Y-m-d H:i:s");
    	return ($date != "" && $date != "1900-01-01 00:00:00") ? $date : null;
    } else {
        return null;
    }
}
...
function get_page_detail ($prety_url) {
    ...
    return $ci->Dynamic_model->set_model("dtb_page", "dp", "id")->get_all_data(
        array(
...
function select_product_category ($name, $selected = FALSE, $html_attr = '', $add_options = array("" => "Select Product Category")) {
    ...
    $ci->load->helper('form');
    $ci->load->model('product/Category_model');

    $datas = $ci->Category_model->get_all_data (array(
        "select" => "id, name"
    ))['datas'];
...
function select_tipe_csr ($name, $selected = FALSE, $html_attr = '', $add_options = array("" => "Select Tipe CSR")) {
    $ci = & get_instance();

    $ci->load->helper('form');
    $ci->load->model('Dynamic_model');

    $datas = Dynamic_model::$tipe_csr;
...
function readmore($str , $maxlength){

    if (strlen($str) > $maxlength) {
        $string  = substr($str, 0, $maxlength).' ...';
    } else {
        $string  = $str;
    }
